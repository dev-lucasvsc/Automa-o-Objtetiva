window.addEventListener('pywebviewready', () => {
    ...
    btnStart.addEventListener('click', () => {
        ...
        disableControls();
        pywebview.api.executar_automacao_principal_tab(dataInicioFormatada, dataFimFormatada);
    });
    ...
    function renderizarResultadosFornecedor(resultados) {
        ...
        const codigosJaSelecionados = fornecedorInput.value.split(',').map(s => s.trim()).filter(Boolean);
    ...
    btnFornecedorSubmit.addEventListener('click', () => {
        ...
        if (!codigos) {
            window.showMessage("Entrada Inválida", "Nenhum código de fornecedor selecionado ou digitado.");
            return;
        }
